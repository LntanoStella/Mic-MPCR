...
#
# 功能:
#   - 读取由 augment_dataset.py 生成的、包含 .npy 文件的增强数据集文件夹。
...
#
# 使用方法 (在终端中运行):
#   python verify_augmented_data.py --folder "path/to/your/Variation_XXXX"
#
# =========================================================================

def main():
    parser = argparse.ArgumentParser(description="深度验证一个增强后的仿真数据组。")
    parser.add_argument('--folder', type=str, default=r'D:\Application\PycharmProfessional\pycharm\PointCloud_registration\GeoTransformer-main\data\custom\random_Dataset_50\MiC_3\Variation_0000',
                        help='要可视化的单个Variation_XXXX文件夹路径。')
    args = parser.parse_args()

    data_folder = args.folder
    print(f"--- 开始深度验证数据组: {data_folder} ---\n")
    ...
    manifest_path = os.path.join(data_folder, 'manifest.txt')

    if not os.path.isdir(pcd_dir) or not os.path.isdir(transform_dir) or not os.path.isfile(manifest_path):
        print("错误: 输入文件夹结构不完整。请确保包含 'point_clouds', 'transforms' 子文件夹和 'manifest.txt' 文件。")
        return
    ...
    for line in lines:
        ...
        try:
            ...
            # 加载变换矩阵
            transform_matrix = np.load(os.path.join(data_folder, trans_rel_path))
